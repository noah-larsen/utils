...
trait Commands extends Enumerated {
  ...
  def promptUntilParsed[T](indexCommandItems: Map[Int, T] = Map[Int, T](), without: Seq[CommandType] = Seq(), showUsage: Boolean = true, leadWithNewline: Boolean = true)
                          (implicit clearScreenUponSuccess: Boolean = true): CommandInvocation[T] = {
  ...
  private def parse[T](line: String, indexedCommandValues: Map[Int, T] = Map(), without: Seq[CommandType] = Seq()): Try[CommandInvocation[T]] = {
    ...
    letterCommands
      .find(_.letterName.toString == tokens.head).filter(!without.contains(_)).map(CommandInvocation[T](_, tokens.tail).validate)
      .orElse(
        indexedCommand.collect {
          ...
          case _: IndexListCommand =>
            indexedCommand.filter(!without.contains(_)).map((_, tokens.map(y => Try(y.toInt)))).filter(_._2.forall(_.isSuccess)).map(x => (x._1, x._2.map(_.get))).filter(_._2
              .forall(x => indexedCommandValues.contains(x))).map(x => Try(CommandInvocation[T](x._1, Seq(), None, Some(x._2.map(indexedCommandValues(_))))))
        }.flatten
      )
