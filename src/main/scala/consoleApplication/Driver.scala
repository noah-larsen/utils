      ...
      commandInvocation.command match {
        case MainCommands.Load =>
          Try {
            ...
            val renaming = intermediateDataDictionary.fieldEntriesObject(IngestionStages.Raw, physicalNameObject.string).flatMap(_.map(Try(_)).orElse(Some(Unit).filter(_ => country == Countries.UnitedStates).flatMap(_ => USSourceSystems
            ...
            table(ConsoleRenamer(renaming.get, targetNames, nameComparator, nameSearch, nTopHitsToGetPossiblyPositiveScores, approvedRenamings.originalToRenamedNameToNOccurences, unapprovedRenamings.originalToRenamedNameToNOccurences))
        ...
        case MainCommands.WriteOnceToDataDictionary =>
          ...
          println(workDocument.entriesObject(physicalNameObject).flatMap(_.map(Try(_)).getOrElse(Failure(DataHubException("Object does not exist in work document")))).map{
            ...
            case x =>
                x.merge(intermediateDataDictionary, preserveRegistrationDatesThis = false, preserveRegistrationDatesThat = false)
