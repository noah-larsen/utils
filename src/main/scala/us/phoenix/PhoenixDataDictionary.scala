...
//        Try(poddRows.filter(_.tableName.equalsIgnoreCase(physicalNameObject.objectName)).map(poddRow =>
//          FieldEntry(
...
//          )
//        )).filter(_.nonEmpty).recoverWith{case e: Exception => Failure(DataHubException(s"No entries for table ${physicalNameObject.objectName} in alnova table layouts"))}
